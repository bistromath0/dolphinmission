...
public class GameObject {
	...
	/**
	 * performs a yaw motion on the game object (keyboard), for direction 1 is left,
	 * -1 is right
	 */
	...
	public void pitch(int direction) {
		Matrix4f oldRotation, rotAroundAvatarX, newRotation;
		oldRotation = new Matrix4f(getLocalRotation());

		float rotationAmount = direction * 0.005f;

		// Create rotation matrix around local X axis (right vector)
		rotAroundAvatarX = new Matrix4f().rotationX(rotationAmount);

		// Combine rotations
		newRotation = oldRotation.mul(rotAroundAvatarX);
		setLocalRotation(newRotation);
	}
