...
public class Camera {
	...
	/** performs yaw action on camera, direction 1 is left -1 is right */
	public void yaw(int direction) {
		Matrix3f rotAroundCamUp;
		float rotationAmount = direction * 0.005f;
		rotAroundCamUp = new Matrix3f().rotation(rotationAmount, getV());
		setU(getU().mul(rotAroundCamUp));
		setN(getN().mul(rotAroundCamUp));
	}

	public void pitch(int direction) {
		Matrix3f rotAroundCamRight;
		float rotationAmount = direction * 0.005f;
		rotAroundCamRight = new Matrix3f().rotation(rotationAmount, getU());
		setV(getV().mul(rotAroundCamRight));
		setN(getN().mul(rotAroundCamRight));
	}
}